...
struct ParamRule {
    std::string name;
    bool required = true;
    std::string type = "string";  // string, int, float, bool, array, object
    ...
    std::vector<std::string> allowedValues; // 允许的值列表
};

// 验证参数
void validateParams(const nlohmann::json &params, const std::vector<ParamRule>& rules);
...
{
public:
    ApiException(ApiErrorCode errorCode, const std::string& errorMsg = "")
    ...
    std::string getErrorMessage() const { return _errorMsg; }
