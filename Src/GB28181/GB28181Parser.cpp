...
void GB28181Parser::parse(const char *data, size_t len)
{
    ...
    while (data < end) {
        ...
        if (len >= _contentLen + 2) {
            onRtpPacket(data, _contentLen + 2);
            // static int i = 0;
...
void GB28181Parser::setOnRtpPacket(const function<void(const char* data, int len)>& cb)
{
    _onRtpPacket = cb;
}

void GB28181Parser::onRtpPacket(const char* data, int len)
{
