...
PacketRecvRateContext::PacketRecvRateContext(uint64_t start)
...
void PacketRecvRateContext::inputPacket(uint64_t &ts,size_t len) {
    auto tmp = ts - _last_arrive_time;
...
EstimatedLinkCapacityContext::EstimatedLinkCapacityContext(uint64_t start) : _start(start) {
    for (size_t i = 0; i < SIZE; i++) {
        _dur_probe_arr[i] = 1000;
    }
    _cur_idx = 0;
};
void EstimatedLinkCapacityContext::inputPacket(uint64_t &ts,DataPacket::Ptr& pkt) {
    uint32_t  seq = pkt->getPacketSeqNumber();
    auto diff  = seqCmp(seq,_last_seq);
    const bool retransmitted = pkt->getR() == 1;
...
void EstimatedLinkCapacityContext::probe1Arrival(uint64_t &ts, const DataPacket::Ptr &pkt, bool unordered) {
    if (unordered && pkt->getPacketSeqNumber() == _probe1_seq) {
    ...
    _ts_probe_time = ts;
    _probe1_seq = pkt->getPacketSeqNumber(); // Record the sequence where 16th packet probe was taken
}

/// Record the arrival time of the second probing packet and the interval between packet pairs.

void EstimatedLinkCapacityContext::probe2Arrival(uint64_t &ts, const DataPacket::Ptr &pkt) {
    ...
    if (_probe1_seq == SEQ_NONE || incSeq(_probe1_seq) != pkt->getPacketSeqNumber())
        return;
    ...
    const int64_t timediff = ts - _ts_probe_time;
    ...
    //
    const size_t pktsz = pkt->getLength() - HDR_SIZE;
