#ifndef RtspCombineMediaSource_H
#define RtspCombineMediaSource_H
...
class RtspCombineMediaSource : public RtspMediaSource
{
public:
    using Ptr = std::shared_ptr<RtspCombineMediaSource>;
    using Wptr = std::weak_ptr<RtspCombineMediaSource>;
    using DataType = std::shared_ptr<std::deque<RtpPacket::Ptr> >;
    using QueType = DataQue<DataType>;

    RtspCombineMediaSource(const UrlParser& urlParser, const EventLoop::Ptr& loop = nullptr, bool muxer = false);
    virtual ~RtspCombineMediaSource();
...
public:
    ...
    void setVideoMediaSource(const UrlParser& urlParser) {_videoUrlParser = urlParser;}
    void setAudioMediaSource(const UrlParser& urlParser) {_audioUrlParser = urlParser;}

    void start(const std::function<void()>& onReady);
    void onAudioSource(const FrameMediaSource::Ptr& source);
    void onVideoSource(const FrameMediaSource::Ptr& source);
...
private:
    ...
    UrlParser _audioUrlParser;

    FrameMediaSource::Ptr _videoMediaSource;
    FrameMediaSource::Ptr _audioMediaSource;
...
#endif //RtspCombineMediaSource_H
