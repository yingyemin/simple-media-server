#ifndef HttpClient_h
#define HttpClient_h

#include "HttpParser.h"
#include "Common/UrlParser.h"
#include "Net/TcpClient.h"

#include <string>
#include <unordered_map>
#include <memory>
#include <functional>

// using namespace std;

class HttpClient : public TcpClient
{
public:
    HttpClient(const EventLoop::Ptr& loop);
    HttpClient(const EventLoop::Ptr& loop, bool enableTls);
    ~HttpClient();

public:
    int start(const std::string& localIp, int localPort, const std::string& peerIp, int peerPort, int timeout);
    int sendHeader(const std::string& localIp, int localPort, const std::string& url, int timeout);
    void setWebsocket() {_isWebsocket = true;}

    virtual void onRead(const StreamBuffer::Ptr& buffer, struct sockaddr* addr, int len) override;
    void onError(const std::string& err) override;
    virtual void close() override;
    void onConnect() override;

    void addHeader(const std::string& key, const std::string& header);
    void setContent(const std::string& content);
    void setMethod(const std::string& method);

    int sendHeader(const std::string& url, int timeout);
    void sendContent(const char* data, int len);
    void send(const std::string& msg);

    virtual void onHttpRequest();
    virtual void onRecvContent(const char *data, uint64_t len);

public:
    std::string _websocketKey;
    HttpParser _parser;
    HttpParser _request;

private:
    bool _isWebsocket = false;
    std::string _body;
    UrlParser _urlParser;
    EventLoop::Ptr _loop;
};

#endif //HttpClient_h