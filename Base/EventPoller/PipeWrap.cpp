...
#define checkFD(fd) \
    if (fd == -1) { \
        clearFD(); \
        throw runtime_error(std::string("Create windows pipe failed: ") + get_uv_errmsg());\
    }
...
void PipeWrap::reOpen() {
    ...
    toolkit::close_my(listener_fd);
#else
    if (pipe(_pipe_fd) == -1) {
        throw runtime_error(string("Create posix pipe failed: ") + get_uv_errmsg());
    }
