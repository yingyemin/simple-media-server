...
int ioctl_my(int fd, long cmd, u_long *ptr) {
...
int SockUtil::setCloseWait(int fd, int second) {
    ...
    if (ret == -1) {
#ifndef _WIN32
        logTrace << "setsockopt SO_LINGER failed";
#endif
    }
    return ret;
}
...
int SockUtil::setNoDelay(int fd, bool on) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt TCP_NODELAY failed";
    }
    return ret;
}
...
int SockUtil::setReuseable(int fd, bool on, bool reuse_port) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt SO_REUSEADDR failed";
        return ret;
    }
...
int SockUtil::setBroadcast(int fd, bool on) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt SO_BROADCAST failed";
    }
    return ret;
}
...
int SockUtil::setKeepAlive(int fd, bool on, int interval, int idle, int times) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt SO_KEEPALIVE failed";
    }
#if !defined(_WIN32)
...
int SockUtil::setNoBlocked(int fd, bool noblock) {
    ...
    int ul = noblock;
#endif //defined(_WIN32)
    int ret = ioctl_my(fd, FIONBIO, &ul); //设置为非阻塞模式
    if (ret == -1) {
...
int SockUtil::setRecvBuf(int fd, int size) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt SO_RCVBUF failed";
    }
    return ret;
}
...
int SockUtil::setSendBuf(int fd, int size) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt SO_SNDBUF failed";
    }
    return ret;
}
...
private:
    ...
    std::shared_ptr<struct addrinfo> getSystemDomainIP(const char *host) {
        ...
        if (!answer) {
            logTrace << "getaddrinfo failed: " << host;
            return nullptr;
        }
...
static int set_ipv6_only(int fd, bool flag) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IPV6_V6ONLY failed";
    }
    return ret;
}
...
static int bind_sock6(int fd, const char *ifr_ip, uint16_t port) {
    ...
    if (::bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
        logWarn << "Bind socket failed: " << get_uv_errmsg(true);
        return -1;
    }
    return 0;
}
...
int SockUtil::connect(const char *host, uint16_t port, bool async, const char *local_ip, uint16_t local_port) {
    ...
    if (sockfd < 0) {
        logWarn << "Create socket failed: " << host;
        return -1;
    }
    ...
    logWarn << "Connect socket to " << host << " " << port << " failed: " << get_uv_errmsg(true);
    close_my(sockfd);
    return -1;
}
...
int SockUtil::listen(const uint16_t port, const char *local_ip, int back_log) {
    ...
    if ((fd = (int)socket(family, SOCK_STREAM, IPPROTO_TCP)) == -1) {
        logWarn << "Create socket failed: " << get_uv_errmsg(true);
        return -1;
    }
    ...
    if (::listen(fd, back_log) == -1) {
        logWarn << "Listen socket failed: " << get_uv_errmsg(true);
        close_my(fd);
        return -1;
    }
...
void for_each_netAdapter_posix(FUN &&fun){ //type: struct ifreq *
    ...
    if (-1 == ioctl_my(sockfd, SIOCGIFCONF, &ifconf)) {    //获取所有接口信息
...
int SockUtil::dissolveUdpSock(int fd) {
    ...
    if (-1 == ::connect(fd, (struct sockaddr *)&addr, addr_len) && get_uv_error() != UV_EAFNOSUPPORT) {
        ...
        logWarn << "Connect socket AF_UNSPEC failed: " << get_uv_errmsg(true);
        return -1;
    }
   return 0;
}
...
string SockUtil::get_ifr_mask(const char *if_name) {
    ...
    if ((ioctl_my(fd, SIOCGIFNETMASK, &ifr_mask)) < 0) {
...
string SockUtil::get_ifr_brdaddr(const char *if_name) {
    ...
    memset(&ifr_mask, 0, sizeof(ifr_mask));
    strncpy(ifr_mask.ifr_name, if_name, sizeof(ifr_mask.ifr_name) - 1);
    if ((ioctl_my(fd, SIOCGIFBRDADDR, &ifr_mask)) < 0) {
        logWarn << "ioctl SIOCGIFBRDADDR failed: " << get_uv_errmsg(true);
        close(fd);
        return "";
    }
    close(fd);
...
int SockUtil::setMultiTTL(int fd, uint8_t ttl) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_MULTICAST_TTL failed";
    }
#endif
...
int SockUtil::setMultiIF(int fd, const char *local_ip) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_MULTICAST_IF failed";
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::setMultiLOOP(int fd, bool accept) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_MULTICAST_LOOP failed";
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::joinMultiAddr(int fd, const char *addr, const char *local_ip) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_ADD_MEMBERSHIP failed: " << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::leaveMultiAddr(int fd, const char *addr, const char *local_ip) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_DROP_MEMBERSHIP failed: " << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::joinMultiAddrFilter(int fd, const char *addr, const char *src_ip, const char *local_ip) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_ADD_SOURCE_MEMBERSHIP failed: " << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::leaveMultiAddrFilter(int fd, const char *addr, const char *src_ip, const char *local_ip) {
    ...
    if (ret == -1) {
        logTrace << "setsockopt IP_DROP_SOURCE_MEMBERSHIP failed: " << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
